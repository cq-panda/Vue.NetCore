...
## 框架核心
 - 快速开发
 - 支持前端、后台自定义扩展
 - 代码生成(20多属性在线配置表单、table)
 - table自动转换key/value
 - 表单select/checkbox自动绑定数据源
 - 支持(主从表)一对一代码生成,一对多从表使用扩展可轻松实现
...
## 开发及依赖环境
VS2019 、.NetCore3.1 、EFCore3.1、JWT、Dapper、Autofac、SqlServer/MySql、Redis(可选，没有redis的在appsetting.json中不用配置，默认使用内置IMemory)、<br>VsCode、Vue2.0（webpack、node.js,如果没有此环境自行搜索:vue webpack npm)、Vuex、axios、promise、IView、Element-ui
