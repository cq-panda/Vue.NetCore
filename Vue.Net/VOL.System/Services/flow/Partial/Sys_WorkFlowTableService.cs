    ...
    public partial class Sys_WorkFlowTableService
    {
        ...
        public override PageGridData<Sys_WorkFlowTable> GetPageData(PageDataOptions options)
        {
            if (!UserContext.Current.IsSuperAdmin)
            {
                ...
                //显示当前用户需要审批的数据
                var stepQuery = _stepRepository.FindAsIQueryable(x => (x.StepType == (int)AuditType.用户审批 && x.AuditId == user.User_Id)
                  || (x.StepType == (int)AuditType.角色审批 && x.AuditId == user.Role_Id)
                  || (x.StepType == (int)AuditType.部门审批 && x.AuditId == user.DeptId)
                   );
